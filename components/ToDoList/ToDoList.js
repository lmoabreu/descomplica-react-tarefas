...
export default class ToDoList extends React.Component {
    ...
    handleDeleteToDoList = (toDoList) => {   
        Alert.alert('Atenção', `Deseja realmente excluir a lista de tarefas "${toDoList.name}"?`, [
            {
                text: 'Cancelar',
                style: 'cancel',
                onPress: () => {}
    ...
    render() {
        ...
        return (
            <View>
                <TouchableOpacity
                    onLongPress={() => this.handleDeleteToDoList(toDoList)}
                    style={[styles.listContainer, {backgroundColor: color}]}
                    onPress={() => navigation.navigate('ToDoScreen', {toDoList, onGoBack})}>
