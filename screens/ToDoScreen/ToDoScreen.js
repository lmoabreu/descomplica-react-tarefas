...
import { SafeAreaView, Text, StyleSheet, TouchableOpacity, View, FlatList, KeyboardAvoidingView, TextInput, Keyboard, Animated, Alert, TouchableHighlight } from 'react-native';
...
export default class ToDoScreen extends React.Component {
    ...
    handleUpdateToDoList = (toDoList) => {
        ...
        updateToDoListService.execute(toDoList).then(() => {
            ...
            Alert.alert('Erro', error.mensage);
        })
    }
    ...
    handleDeleteButton = (index) => {
        ...
        Alert.alert('Atenção!', `Deseja realmente excluir a tarefa "${toDoList.todos[index].title}"?`,
        [
            {
                text: "Cancelar",
                style: "cancel",
        ...
        return (
            <TouchableOpacity onPress={() => this.handleDeleteButton(index)}>
                <Animated.View style={[styles.deleteButton, {opacity}]}>
                    <AnimatedIcon name="delete" size={25} color="#FFF" style={[{'transform': [{scale}]}]}></AnimatedIcon>
                </Animated.View>
            </TouchableOpacity>
        );
            ...
            <Swipeable
                ...
                <View style={styles.toDoItemContainer}>
                    <TouchableOpacity onPress={() => this.toogleToDoCompleted(index)}>
                        <Ionicons name={completed ? 'ios-square' : 'ios-square-outline'} size={30} color={'#D3D3D'} style={{width: 35}}/>
                    </TouchableOpacity>
                    <Text style={[styles.toDoItemText, {
                        color: completed ? '#D3D3D3' : '#000',
            ...
            <KeyboardAvoidingView style={{flex: 1}} behavior="scroll">
                <SafeAreaView style={styles.container}>
                    ...
                    <View style={[styles.section, styles.footer]}>
                        <TextInput
                            style={[styles.textInput, {borderColor: toDoList.color}]}
                            onChangeText={text => this.setState({newToDo: text})}
                            value={this.state.newToDo} />
                        <TouchableOpacity
                            style={[styles.addToDoButton, {backgroundColor: toDoList.color}]}
